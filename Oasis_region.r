...
region <- '/project/Spatial_OASIS/Region/'
...
for (i in 1:length(training)){
  ...
  #save(list = name,file=paste0(region,"training_", subject.id, "_dataframe", ".rda"))
...
for (i in 1:length(training)){
  ...
  print(paste("Beginning: ",subject.id))
  #load(file=paste0(region,"training_", subject.id, "_dataframe", ".rda"))
  ...
  #save(list = out_name,file=paste0(region,"training_", subject.id, "_dataframe", ".rda"))
...
# for (i in 1:length(training)) {
#   subject.id  <- training[i]
#   load(file= paste0("/project/Spatial_OASIS/Region/training_", subject.id, "_dataframe.rda"))
# }
#
...
#load(file=paste0(region,"train_oasis_model_by_region.rda"))
...
##########################################################################################
## Random intercepts OASIS predictions on testing dataset - by region
##########################################################################################
...
for (i in 1:length(testing)){
  ...
  EVE = readnii(filepaths_test$EVE[i])
    ...
    # Create training dataframe for ith subject 
  out <- paste("oasis_regpred_", subject.id, sep="")
  assign(out,oasis_predict_region(flair = FLAIR,t1 = T1,t2 = T2,pd = PD,brain_mask = brain_mask, EVE = EVE, preproc=FALSE,normalize=TRUE,binary=TRUE,cores=4))
  #save(list = out, file=paste0(region,"oasis_regpred_", subject.id, ".rda"))
  print(paste("Finished: ",subject.id))
}
#ls(pattern="pred_")

# Add a column of brain labels to dataframe for each subject
# for (i in 1:length(testing)){
#   subject.id  <- testing[i]
...
#   # Retrieve ith subject's trained dataframe
#   trained_dataframe <- get(ls(pattern=paste("pred_", subject.id, sep="")))
...
#   # Output new dataframe
#   out_name <- paste("pred_", subject.id, "_dataframe", sep="")
#   assign(out_name,trained_dataframe)
#   #save(list = out_name, file=paste0(region,"pred_", subject.id, ".rda"))
  
#   print(paste("Finished: ",subject.id))
  
# }

# # Now, for EACH subject we make separate predictions for EACH brain region.
# for (i in 1:length(testing)){
#   subject.id  <- testing[i]
  
#   print(paste("Beginning: ",subject.id))
#   #load(file=paste0(region,"pred_", subject.id, ".rda"))
#   # Retrieve ith subject's trained dataframe
#   trained_dataframe <- get(ls(pattern=paste("pred_", subject.id, "_dataframe", sep="")))
  
#   # List of regions for this subject, exclude region 0 ('background')
#   region_list <- sort(unique(trained_dataframe$oasis_dataframe$region))
#   region_list <- region_list[region_list!=0]
  
#   # Add row number so we can resort to original order, then sort by region id
#   trained_dataframe$oasis_dataframe$order <- 1:nrow(trained_dataframe$oasis_dataframe)
#   trained_dataframe$oasis_dataframe <- trained_dataframe$oasis_dataframe[order(trained_dataframe$oasis_dataframe$region),]
 
#   trained_dataframe$oasis_dataframe$rpred <- NA
  
#   # Loop through regions
#   for (r in 1:length(region_list)){
#     region.id  <- region_list[r]
    
#     # Make model predictions for region
#     rdata  <- trained_dataframe$oasis_dataframe[trained_dataframe$oasis_dataframe$region==region.id,]
#     rmodel <- oasis_region[[r]]
#     rpred  <- predict( rmodel,
#                        newdata = rdata,
#                        type = 'response')
#     trained_dataframe$oasis_dataframe$rpred[trained_dataframe$oasis_dataframe$region==region.id] <- rpred
#   }
#   # Sort back to original order
#   trained_dataframe$oasis_dataframe <- trained_dataframe$oasis_dataframe[order(trained_dataframe$oasis_dataframe$order),]
#   # Put the predictions onto the brain
#   predictions_nifti <- niftiarr(trained_dataframe$brain_mask, 0)
#   predictions_nifti[trained_dataframe$voxel_selection==1] <- trained_dataframe$oasis_dataframe$rpred
...
#   prob_map <- fslsmooth(predictions_nifti, sigma = 1.25, mask = trained_dataframe$brain_mask, retimg = TRUE, smooth_mask = TRUE, verbose = TRUE)
#   writenii(prob_map, file = paste0('/project/Spatial_OASIS/Region/pm_region_', subject.id))
#   # Threshold smoothed predictions
#   binary_map <- prob_map
#   binary_map[prob_map > 0.16] <- 1
#   binary_map[prob_map <= 0.16] <- 0
  
#   # Output
#   L = list(oasis_map = prob_map,
#            binary_map = binary_map,
#            unsmoothed_map = predictions_nifti,
#            flair = trained_dataframe$flair,
#            t1 = trained_dataframe$t1, t2 = trained_dataframe$t2,
#            pd = trained_dataframe$pd,
#            brain_mask = trained_dataframe$brain_mask,
#            voxel_selection = trained_dataframe$voxel_selection)
  
#   out <- paste("val_", subject.id, "_df", sep="")
#   assign(out,L)
#   save(list = out, file=paste0(region,"oasis_byregion_pred_subject_", subject.id, ".rda"))
...
##########################################################################################
## Calculate AUC, partial AUC, and Dice similarity coefficient on regionwise predictions
##########################################################################################

# First, need to create a list of all the subject/region dataframes created above
# for (i in 1:length(testing)) {
#   subject.id  <- testing[i]
#   load(file=paste0(region,"oasis_regpred_", subject.id, ".rda"))
# }
sub_reg_preds <- ls(pattern="oasis_regpred_")
testing_region <- data.frame(subject=rep(NA,length(sub_reg_preds)),
                                AUC=rep(NA,length(sub_reg_preds)),
                                pAUC=rep(NA,length(sub_reg_preds)),
                                Dice=rep(NA,length(sub_reg_preds)))


for (i in 1:length(sub_reg_preds)){
  
  # Extract subject and region IDs from dataframe
  subject.id = substr(sub_reg_preds[i], 15,25)
  testing_region[i,"subject"] <- subject.id
  
  print(paste("Beginning: Subject ",subject.id))
  
  # Read in ith subject's lesion mask
  gold_standard = readnii(filepaths_test$GS[i])
  GS_flip <- neurobase::flip_img(gold_standard , x=TRUE, y = TRUE)

 # Retrieve ith subject's predictions
  L <- get(sub_reg_preds[i])
  brain_mask = readnii(filepaths_test$brainmask[i])
  prob_true_GS <- GS_flip[brain_mask==1]
  probability_map <- L[["oasis_map"]]
  #writenii(probability_map, file = paste0(region,"pm_region_" ,subject.id))
  prob_true_predict <- probability_map[brain_mask==1]
  ROC_obj = roc(as.vector(prob_true_GS), as.vector(prob_true_predict))
  testing_region[i,"AUC"] <- auc(ROC_obj)[1]
  testing_region[i,"pAUC"] <- auc(ROC_obj,partial.auc=c(1,0.99))[1]  
  ...
  FN = dice_tab[2,3]
  DSC <- (2*TP)/((2*TP)+FP + FN)
  testing_region[i,"Dice"] <- DSC
  print(subject.id)
  print(testing_region[i,"AUC"])
  print(testing_region[i,"pAUC"])
  print(DSC)
}


# Calculate AUC and partial AUC
mean(testing_region$AUC)
mean(testing_region$pAUC)/0.01
mean(testing_region$Dice)
df <- data.frame(seed,mean(testing_region$AUC),mean(testing_region$pAUC)/0.01,mean(testing_region$Dice))
write.table(df, file = paste0(region,"results_region.csv"), sep = ',' , append = TRUE, quote = FALSE,col.names = FALSE, row.names = FALSE)
